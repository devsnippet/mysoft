...
angular.module('xenon.services', []).
	service('$menuItems', function()
	{
		...
		this.prepareSidebarMenu = function()
		{
			...
			var system  	 	 = this.addItem('系统设置', 			'/app/system', 				'linecons-cog').setLabel('新项目', 'purple');
				
				// Subitems of Dashboard
				dashboard.addItem('Dashboard 1', 	'-/variant-1'); // "-/" will append parents link
			
				var system_user = system.addItem('用户管理', 	'-/user');
				var system_authority= system.addItem('权限管理', 	'-/authority');
					
				// 子菜单的	系统设置/用户模块
					system_user.addItem('用户列表', 	'-/list');
					system_user.addItem('添加用户', 	'-/add');

					// 子菜单的	系统设置/权限模块
					system_authority.addItem('权限列表', 	'-/authority');
