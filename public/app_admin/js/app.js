...
app.config(function($stateProvider, $urlRouterProvider, $ocLazyLoadProvider, ASSETS){
	...
	$urlRouterProvider.otherwise('login'); //入口文件
	...
	$stateProvider.
		...
		state('app', {
			abstract: true,
			url: '/app',
			templateUrl: appHelper.templatePath('layout/app_body'),
		...
		state('app.login', {
			...
			templateUrl: appHelper.templatePath('dashboards/variant_1'),
			resolve: {
		...
		state('app.system-user-list', {
			...
			templateUrl: appHelper.templatePath('user/lists')
		}).
		//系统设置-用户管理-列表
		state('app.system-user-add', {
			url: '/system-user-add',
			templateUrl: appHelper.templatePath('user/add')
		}).
